...
module.exports = {
  ...
  async execute(client2, message) {
    try {
      ...
      try {
        let commandFiles = client2.commands.get(command) || client2.commands.find(cmd => cmd.aliases && cmd.aliases.includes(command));
        ...
        if (commandFiles) {
          if (commandFiles.cooldown) {
            ...
            commandFiles.execute(client2, message, args);
