...
module.exports = {
  ...
  async execute(client5, message) {
    try {
      ...
      try {
        let commandFiles = client5.commands.get(command) || client5.commands.find(cmd => cmd.aliases && cmd.aliases.includes(command));
        ...
        if (commandFiles) {
          if (commandFiles.cooldown) {
            ...
            commandFiles.execute(client5, message, args);
