...
module.exports = {
  ...
  async execute(client3, message) {
    try {
      ...
      try {
        let commandFiles = client3.commands.get(command) || client3.commands.find(cmd => cmd.aliases && cmd.aliases.includes(command));
        ...
        if (commandFiles) {
          if (commandFiles.cooldown) {
            ...
            commandFiles.execute(client3, message, args);
